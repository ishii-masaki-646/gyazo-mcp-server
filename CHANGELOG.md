...
- Windows / macOS で `cargo build` / `cargo install` 時に `constant SERVICE_NAME is never used` の dead_code 警告が出ていた問題を修正しました。`SERVICE_NAME` 定数は Linux の systemd unit 名 / `systemctl` 引数でしか使っていないため、`#[cfg(target_os = "linux")]` でガードして他 OS では存在しないようにしました。
- CI の clippy ジョブを `ubuntu-latest` / `macos-latest` / `windows-latest` の matrix 構成に変更しました。これまで clippy ジョブが ubuntu-latest 限定だったため、Windows / macOS でだけ発生する dead_code 警告が CI で検出できず手元検証まで気付けない問題がありました (上記 SERVICE_NAME の警告がまさに該当)。再発防止として OS matrix 化します。あわせて CI のトリガーに `test/*` ブランチへの push を追加しました。リリース前に他 OS の CI 結果を実機なしで確認するためのリハーサル用ブランチとして使えます。
- Windows ビルドで `service::tests::uninstall_ps1_warns_about_running_listener_processes_without_stopping_them` テストが失敗していた問題を修正しました。生成 ps1 内のヒント表示用 `Write-Host '停止例: Stop-Process -Id <PID> -Force'` 文字列リテラルにテスト assertion `!ps1.contains("Stop-Process")` が誤マッチしていたため、`Stop-Process` を含む行は `Write-Host` (= ヒント表示) で始まることだけを確認する形に厳密化しました。あわせて main.rs の test mod で import していた `service_runtime_port_hint` が Windows ビルドで unused 警告になっていた問題も修正しました (Windows では参照する回帰テストが `#[cfg(not(target_os = "windows"))]` で除外されるため、import 自体も同じ cfg ガードで条件付きにしました)。
- CI / リリースワークフローで使っている GitHub Actions の major バージョンを最新に更新しました: `actions/checkout` v5 → v6、`actions/upload-artifact` v6 → v7、`actions/download-artifact` v6 → v8、`docker/setup-qemu-action` v3 → v4、`docker/setup-buildx-action` v3 → v4、`docker/login-action` v3 → v4、`docker/build-push-action` v6 → v7。各 release notes を確認し、本リポジトリの呼び出しに影響する破壊的変更が含まれていないこと (廃止された input/env を未使用、Node.js 24 ランタイム要件は GitHub-hosted runner で自動充足される等) を確認したうえで一括更新しました。
