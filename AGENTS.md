...
## 設定とセキュリティ
`config.toml` や `.env` に秘密情報、API トークン、端末固有の値をコミットしないこと。.env 系ファイルはローカル専用として扱って、必要なら `.env.example` だけを共有してね。HTTP transport の MCP クライアント設定では `env` が使えない場合があるから、`GYAZO_MCP_TCP_PORT`、`GYAZO_MCP_OAUTH_CALLBACK_PATH`、`client_id`、`client_secret`、`PAT` はまとめて `~/.config/gyazo-mcp-server/.env` から読む前提にすると運用しやすいわ。設定例が必要なら、実値ではなくプレースホルダーを使って、必要なキーは PR か README に書いて共有すること。
